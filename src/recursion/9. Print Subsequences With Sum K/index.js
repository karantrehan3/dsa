 ...
 * @returns {void}
 */
function printSubsequences(index, arr, currentSum, sum, array) {
  ...
  printSubsequences(index + 1, arr, currentSum, sum, array); // pick

  arr.pop();
  currentSum -= array[index];

  printSubsequences(index + 1, arr, currentSum, sum, array); // not-pick
 ...
 * array = [1, 2, 1]
 */
printSubsequences(0, [], 0, 2, [1, 2, 1]); // invoking call
